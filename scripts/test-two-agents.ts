...
async function main(): Promise<void> {
  console.log('=== Claw Poker 2-Agent Test ===\n');

  const agent1 = createAgent('Agent1');
  const agent2 = createAgent('Agent2');

  // Step 1: Connect and authenticate both agents
  console.log('--- Step 1: Connect & Authenticate ---');
  await Promise.all([connectAndAuth(agent1), connectAndAuth(agent2)]);
  console.log('Both agents authenticated.\n');

  // Step 2: Join queue
  console.log('--- Step 2: Join Matchmaking Queue ---');
  await joinQueue(agent1);
  // Small delay to ensure first agent is processed
  await new Promise(r => setTimeout(r, 1000));
  await joinQueue(agent2);
  console.log('Both agents in queue.\n');
