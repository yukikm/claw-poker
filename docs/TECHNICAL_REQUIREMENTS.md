...
pub struct Game {
    ...
    pub pot: u64,
    pub current_bet: u64,
    pub dealer_position: u8,     // 0 = player1, 1 = player2
    pub current_turn: Pubkey,
    pub phase: GamePhase,        // Preflop, Flop, Turn, River, Showdown
    pub winner: Option<Pubkey>,
    pub status: GameStatus,      // WaitingForPlayers, InProgress, Completed
    pub created_at: i64,
    pub bump: u8,
}
```
...
pub struct PlayerState {
    pub player: Pubkey,
    pub game_id: u64,
    pub hole_cards: [Card; 2],   // TEE内でのみ本人がアクセス可能
    pub chips: u64,
    pub current_bet: u64,
    pub total_bet_in_round: u64,
    pub has_folded: bool,
    pub has_acted: bool,
    pub is_all_in: bool,
    pub bump: u8,
...
pub struct BettingPool {
    pub game_id: u64,
    pub total_pool: u64,
    pub player1_pool: u64,      // Player1に賭けられた総額
    pub player2_pool: u64,      // Player2に賭けられた総額
    pub betting_closed: bool,   // ゲーム開始後にtrue
    pub settled: bool,          // 配当完了後にtrue
    pub bets: Vec<Bet>,
    pub bump: u8,
}
...
pub struct Bet {
    pub bettor: Pubkey,
    pub amount: u64,
    pub backed_player: Pubkey,   // どちらのプレイヤーに賭けたか
}
```
...
|-------------|------|---------------|
| `join_queue` | マッチメイキングキューへのエントリー | ゲーム参加時 |
| `initialize_game` | Game, PlayerState, Vault, BettingPoolアカウントの作成 | マッチング成立時 |
| `create_permission` | PlayerStateのACL設定（Permission ProgramへCPI） | ゲーム初期化直後 |
| `delegate_pda` | Game, PlayerStateアカウントのTEE委譲（Delegation ProgramへCPI） | ACL設定直後 |
| `place_bet` | 観戦者がBettingPoolにベット | ゲーム開始前 |
| `close_betting` | ベット受付の締め切り | ゲーム開始時 |
| `resolve_game` | Vaultから勝者へSOL送金、配当計算・配布 | commit_game完了後 |
| `claim_winnings` | 観戦ベッティングの配当受取 | resolve_game完了後 |

### 5.3 TEE接続で実行するInstruction

| Instruction | 説明 | 実行タイミング |
|-------------|------|---------------|
| `shuffle_and_deal` | カードシャッフルとホールカード配布 | ゲーム開始時 |
| `deal_community_cards` | コミュニティカード配布（Flop: 3枚, Turn: 1枚, River: 1枚） | 各フェーズ開始時 |
| `player_action` | プレイヤーアクション（Fold, Check, Call, Raise, All-in） | プレイヤーの手番 |
| `evaluate_hands` | ハンド評価とショーダウン処理 | ショーダウン時 |
| `commit_game` | ゲーム結果の確定とアカウント返却（commit_and_undelegate CPI） | ゲーム終了時 |
...
2. エージェントA → join_queue TX送信 → L1 (エントリー料支払い)
3. ゲームサーバー → L1のMatchmakingQueueを監視

4. AIエージェントB → WS接続確立 → ゲームサーバー
5. エージェントB → join_queue TX送信 → L1 (エントリー料支払い)
6. ゲームサーバー → 2エージェント検出

7. ゲームサーバー → initialize_game TX → L1
   (Game, PlayerState x2, Vault, BettingPool 作成)

8. ゲームサーバー → create_permission TX x2 → L1
   (player1StatePda: ACL=[player1], player2StatePda: ACL=[player2])

9. ゲームサーバー → delegate_pda TX x3 → L1
   (gamePda, player1StatePda, player2StatePda をTEEに委譲)

10. ゲームサーバー → close_betting TX → L1
    (ベット受付の締め切り)

11. ゲームサーバー → WS通知: { type: "game_joined", game_id, opponent }
    → エージェントA, エージェントB

12. ゲームサーバー → shuffle_and_deal TX → TEE
    (カードシャッフル & ホールカード配布)
...
```
seed = SHA256(client_random + game_id + player1_pubkey + player2_pubkey)
```
...
### 10.2 コミット戦略: PerRound

本プロジェクトでは `PerRound` コミット戦略を採用する。

- **コミットタイミング**: ゲーム終了時（commit_game）のみL1にコミット
- **観戦者向けリアルタイム更新**: WS経由でゲームサーバーからブロードキャスト（L1コミットとは独立）
- **理由**: L1トランザクションコストの最小化と、TEE上でのゲーム速度を最大化

```
PerRoundの場合のL1トランザクション:
  1. initialize_game (1 TX)
  2. create_permission x2 (2 TX)
  3. delegate_pda x3 (3 TX)
  4. close_betting (1 TX)
  5. resolve_game (1 TX)
  合計: 約8 TX/ゲーム + ベットTX (観戦者数に依存)
```
