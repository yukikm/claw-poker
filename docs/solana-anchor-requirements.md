...
│   │   ├── close_betting_pool.rs       # AllIn検出時にL1のBettingPoolを締め切る
│   │   ├── resolve_game.rs
│   │   ├── claim_betting_reward.rs
│   │   ├── shuffle_and_deal.rs         # TEE内実行
│   │   ├── player_action.rs            # TEE内実行
│   │   ├── reveal_community_cards.rs   # TEE内実行
│   │   ├── settle_hand.rs              # TEE内実行（ハンド後チップ再配）
│   │   ├── start_new_hand.rs           # TEE内実行（次ハンド準備）
...
**スペース計算**: 8 (discriminator) + 8 (game_id) + 32 (player1) + 32 (player2) + 8 (buy_in) + 8 (pot) + 32 (current_turn) + 1 (phase) + 5 (board_cards) + 32 (deck_seed) + 8 (player1_committed) + 8 (player2_committed) + 8 (hand_number) + 1 (dealer_position) + 8 (current_small_blind) + 8 (current_big_blind) + 8 (player1_chip_stack) + 8 (player2_chip_stack) + 1 (consecutive_timeouts_p1) + 1 (consecutive_timeouts_p2) + 8 (last_raise_amount) + 8 (last_checkpoint_hand) + 33 (winner: Option<Pubkey>) + 1 (betting_closed) + 8 (created_at) + 1 (bump) = **286 bytes**

**重要**: `space = 8 + std::mem::size_of::<Game>()` ではなく `Game::INIT_SPACE` を使用すること。`std::mem::size_of` はRustのメモリレイアウト（アラインメントパディング）のサイズを返すため、Borshシリアライズサイズと異なる場合がある。Anchorの `#[account]` マクロが生成する `INIT_SPACE` 定数を使用することで正確なBorshサイズが保証される:
```rust
...
**スペース計算**: 8 (discriminator) + 8 (game_id) + 32 (player) + 2 (hole_cards) + 8 (chip_stack) + 8 (chips_committed) + 8 (chips_in_pot_this_hand) + 1 (is_folded) + 1 (is_all_in) + 1 (bump) = **77 bytes**（discriminator込み）

**重要**: `space = 8 + std::mem::size_of::<PlayerState>()` ではなく `8 + PlayerState::INIT_SPACE` を使用すること。
...
3. Gameフィールドを初期化:
   ...
   - `pot = 0`（ハンド開始時にブラインド投入で加算される）
   - `current_turn = player1`（ハンド1のSBはPlayer1）
   - `phase = GamePhase::Waiting`
   - `board_cards = [255; 5]`（全て未公開）
   - `betting_closed = false`
   - `created_at = Clock::get()?.unix_timestamp`
   - `hand_number = 0`（shuffle_and_dealで1にインクリメント）
   - `dealer_position = 0`（Player1がハンド1のディーラー/SB）
   - `current_small_blind = 10`（初期スモールブラインド: 10チップ）
   - `current_big_blind = 20`（初期ビッグブラインド: 20チップ）
   - `player1_chip_stack = 1000`（初期チップ数）
   - `player2_chip_stack = 1000`（初期チップ数）
   - `consecutive_timeouts_p1 = 0`
   - `consecutive_timeouts_p2 = 0`
   - `last_raise_amount = 0`
   - `last_checkpoint_hand = 0`
4. PlayerStateフィールドを初期化:
   - `hole_cards = [255; 2]`（未配布）
   - `chip_stack = 1000`（初期チップ数）
   - `chips_committed = 0`
   - `chips_in_pot_this_hand = 0`
   - `is_folded = false`
   - `is_all_in = false`
...
pub struct PlaceSpectatorBet<'info> {
    ...
    pub betting_pool: Account<'info, BettingPool>,
    /// Game account（補助チェック。TEEとL1の同期ラグがあるため、主な締め切り制御はclose_betting_pool instructionで行う）
    #[account(
        seeds = [b"game", game_id.to_le_bytes().as_ref()],
        bump = game.bump,
    )]
...
#### 4.1.12 `close_betting_pool(game_id: u64)`

ゲームサーバー（クランク）がAll-in検出時にL1上のBettingPoolを即座に締め切る。TEEとL1の同期ラグを解消するために必要。

**設計背景**: `player_action`でAll-inが発生した場合、Gameアカウント（TEE委譲中）の`betting_closed`フラグはcommit_gameが実行されるまでL1に反映されない。このラグの間、`place_spectator_bet`がBettingPoolへのベットを受け付けてしまうため、ゲームサーバーがオフチェーンでAll-inを検知した段階でこのinstructionを呼び出す。
...
pub struct CloseBettingPool<'info> {
    ...
    pub betting_pool: Account<'info, BettingPool>,
    /// CHECK: ゲームサーバーの権限キー（クランク）
    #[account(
        constraint = game_server.key() == GAME_SERVER_PUBKEY @ PokerError::PermissionDenied
    )]
...
**処理**:
1. `game_server`がゲームサーバーの権限キーであることを確認（`PermissionDenied`エラー）
2. `betting_pool.is_closed == false`を確認（既に締め切り済みならエラー）
3. `betting_pool.is_closed = true`に設定
4. 以降の`place_spectator_bet`はこのBettingPoolに対して失敗する

**呼び出しタイミング**: ゲームサーバーがTEEからのAll-inイベント（WebSocketまたはPERのイベントストリーム）を受信した直後に呼び出す。

#### 4.1.13 `resolve_game(game_id: u64)`
...
5. 配当を計算（Pari-mutuel方式）:
   ...
   ```
   **丸め誤差による資金不足対策**: 整数除算の切り捨てにより`net_payout`が BettingPool の残高を超える可能性を防ぐため、実際の転送額は `min(net_payout, betting_pool_balance)` を使用する。これにより最後のクレーマーも必ず成功する。
6. `bet_record.claimed = true`に設定（再入攻撃防止: Effects before Interactions）
7. BettingPool PDAの残高を確認し、`actual_payout = min(net_payout, pool_balance)` を計算
8. BettingPool PDAの署名でSOLをベッターに転送（`actual_payout`）
9. プラットフォーム手数料をtreasury宛に転送（`platform_fee` または残高に応じて調整）
...
**処理**:
1. `game.hand_number += 1`（ハンド番号をインクリメント）
2. VRFシードを生成（詳細は第7章参照）:
   ```
   seed = SHA256(random_seed || game_id.to_le_bytes() || player1.to_bytes() || player2.to_bytes() || hand_number.to_le_bytes())
   ```
   **注**: hand_numberをシードに含めることで各ハンドで異なるデッキを保証する。
3. `game.deck_seed = seed`に保存
4. Fisher-Yatesシャッフルでデッキ（0-51）をシャッフル
5. ホールカード配布:
   - `player1_state.hole_cards = [deck[0], deck[1]]`
   - `player2_state.hole_cards = [deck[2], deck[3]]`
6. ブラインド投入:
   - dealer_position == 0 の場合: player1がSB(10チップ), player2がBB(20チップ)
   - dealer_position == 1 の場合: player2がSB(10チップ), player1がBB(20チップ)
   - `chips_committed`, `chips_in_pot_this_hand` を更新、`game.pot += SB + BB`
7. `game.phase = GamePhase::PreFlop`に設定
8. `game.current_turn = SBプレイヤー`（ヘッズアップではSBが先手）
9. ブラインドレベルの更新（hand_numberに応じてcurrent_small/big_blindを更新）:
   - hand 1-50: SB=10, BB=20
   - hand 51-100: SB=20, BB=40
   - hand 101-150: SB=30, BB=60
   - hand 151以降（200ハンド上限後のタイブレーク含む）: SB=50, BB=100
...
3. アクションに応じた処理:
   ...
   **Fold**:
   ...
   - **`game.phase`は直接`Finished`にしない。** `settle_hand`を経由してチップ精算を行うため、フォールドの場合は `game.phase = GamePhase::Showdown` に設定し（または現在のフェーズのまま）、`settle_hand`が`is_folded == true`を検知してポットを相手に付与する
   - `game.current_turn`を`FOLD_SENTINEL`（ゼロアドレス）に設定してベッティングラウンド終了を示す

   **Check**:
   - 現在のベット額が相手と同額であることを確認（`opponent_committed == player_committed`）
   - ベット額の変更なし
   - 相手もCheckかCall済みの場合（両者committed一致）: 次のフェーズに進む（後述）

   **Call**:
   - 相手のコミット額との差分を計算
   - `call_amount = opponent_committed - player_committed`
   - スタック不足の場合は `call_amount = player_state.chip_stack` として自動All-inに変換
   - `player_state.chips_committed += call_amount`（checked_add）
   - `player_state.chip_stack -= call_amount`（checked_sub）
   - `game.pot += call_amount`（checked_add）
   - Gameのcommittedフィールドを更新

   **Bet**:
   - `amount`が必須（`InvalidAction`エラー）
   - 現在ベットが0であることを確認（`player1_committed == 0 && player2_committed == 0`）
   - `amount >= current_big_blind`を確認（最小ベット: 1BB）
   - `amount <= player_state.chip_stack`を確認（`InsufficientChips`エラー）
   - `player_state.chips_committed += amount`（checked_add）
   - `player_state.chip_stack -= amount`（checked_sub）
   - `game.pot += amount`（checked_add）
   - `game.last_raise_amount = amount`に更新
   - Gameのcommittedフィールドを更新

   **Raise**:
   - `amount`が必須（これは**レイズ先のtotal committed額**を指す）
   - 例: BBが20、SBが60にレイズする場合 `amount = 60`
   - **最小レイズ検証**: `amount >= opponent_committed + game.last_raise_amount`（`InvalidRaise`エラー）
     - 例: 相手がBet40（last_raise_amount=40）の場合、最小レイズ = 40 + 40 = 80
   - `already_committed = player_committed`
   - `raise_increment = amount - opponent_committed`（今回のレイズ増分）
   - `total_to_add = amount - already_committed`（追加投入額）
   - `total_to_add <= player_state.chip_stack`を確認（`InsufficientChips`エラー）
   - `player_state.chips_committed = amount`（checked_add）
   - `player_state.chip_stack -= total_to_add`（checked_sub）
   - `game.pot += total_to_add`（checked_add）
   - `game.last_raise_amount = raise_increment`に更新
   - Gameのcommittedフィールドを更新

   **AllIn**:
   - `all_in_amount = player_state.chip_stack`（残り全チップ）
   - `player_state.chips_committed += all_in_amount`（checked_add）
   - `player_state.chip_stack = 0`
   - `game.pot += all_in_amount`（checked_add）
   - `player_state.is_all_in = true`
   - `game.betting_closed = true`（TEE内フラグ: ゲームサーバーが検知してclose_betting_poolを呼ぶ）
   - Gameのcommittedフィールドを更新

4. ターンを相手プレイヤーに切り替え（`game.current_turn = opponent`）
5. **ベッティングラウンド終了判定**:
   - 両プレイヤーのcoommitted額が一致した場合（Call完了 or 両者Check）:
     - Fold発生中: `settle_hand`へ（ゲームサーバーが呼び出す）
     - All-in完了: 残りコミュニティカードを全公開してShowdownへ
     - 通常終了: 次のストリートへ（`reveal_community_cards`をゲームサーバーが呼び出す）
   - Riverのベッティング完了（Foldなし）: `game.phase = GamePhase::Showdown` に設定

**フェーズ遷移まとめ（player_action内での自動遷移）**:
- PreFlop → Fold: `settle_hand`に委ねる（Foldフラグで判定）
- PreFlop → ベッティング完了: ゲームサーバーが `reveal_community_cards(Flop)` を呼び出す
- Flop/Turn → ベッティング完了: ゲームサーバーが `reveal_community_cards(Turn/River)` を呼び出す
- River → ベッティング完了（Foldなし）: `game.phase = GamePhase::Showdown` に設定
- Any → All-in + Call: `game.phase = GamePhase::Showdown`（残りカードをrunout）

**AllIn検出時のBettingPool連動**:
- `player_action`内でAllInが実行された場合、Gameの`betting_closed`フラグをtrueに設定
- ゲームサーバーがTEEからこのフラグを検知し、L1上で`close_betting_pool`を呼び出してBettingPoolを即座に締め切る
...
3. `game.phase`を更新（Flop/Turn/River）
4. `player1_committed`と`player2_committed`をリセット（新しいベッティングラウンド）
5. `game.last_raise_amount = 0`をリセット
6. **Postflopのアクション順序設定**: `game.current_turn = BBプレイヤー`（ヘッズアップPostflopではBB=非ディーラーが先手）
   - `dealer_position == 0`（Player1がSB/ディーラー）の場合: `current_turn = player2`（Player2がBB）
   - `dealer_position == 1`（Player2がSB/ディーラー）の場合: `current_turn = player1`（Player1がBB）

#### 4.2.4 `settle_hand(game_id: u64)`

ハンド終了後にチップを再分配し、次のハンドに向けてハンドレベルの状態をリセットする。フォールドまたはショーダウン後に呼び出す。
...
**処理**:
1. **Effective Stack調整（All-inの超過チップ返却）**:
   - いずれかのプレイヤーが`is_all_in == true`の場合:
     - `effective_stack = min(player1_state.chips_in_pot_this_hand, player2_state.chips_in_pot_this_hand)`
     - 超過チップ = `chips_in_pot_this_hand - effective_stack`（超過した側）
     - 超過分を該当プレイヤーの`chip_stack`に返却: `chip_stack += excess`（checked_add）
     - `game.pot` から超過分を差し引く: `game.pot -= excess`（checked_sub）
   - これにより `game.pot == effective_stack * 2`（ヘッズアップ）が保証される

2. **ハンド勝者を決定**:
   - `player_state.is_folded == true`の場合: Foldしていない側が勝者
   - `game.phase == GamePhase::Showdown`の場合: **ハンド評価を実行**:
     - 各プレイヤーの7枚（hole_cards[2] + board_cards[5]）から最強5枚を評価（`evaluate_hand`関数）
     - ハンドランキングを比較し勝者を決定
     - 引き分けの場合はポットを二等分（オッドチップはBBプレイヤーに付与）

3. ポット（`game.pot`）を勝者の`chip_stack`に加算（checked_add）
   - 引き分けの場合: 各プレイヤーに`pot / 2`を付与、オッドチップはBBプレイヤーに

4. チップ保全不変条件の確認: `player1.chip_stack + player2.chip_stack == 2000`
   - 一致しない場合はエラー（`PotOverflow`）

5. `game.player1_chip_stack`, `game.player2_chip_stack`を更新（フロントエンド向けミラー）
...
6. 各PlayerStateのリセット:
   ...
   - `is_folded = false`
   - `is_all_in = false`
   - `hole_cards = [255; 2]`（次のハンドまで非公開）

7. `game.pot = 0`（次のハンド用にリセット）
8. `game.board_cards = [255; 5]`（次のハンド用にリセット）
9. `game.phase = GamePhase::Waiting`
10. いずれかのプレイヤーの`chip_stack == 0`の場合、`game.phase = GamePhase::Finished`を設定

#### 4.2.5 `start_new_hand(game_id: u64)`

`settle_hand`後に次のハンドを開始する。ディーラーを交代し、状態を次のハンドに向けて準備する。
...
**処理**:
1. `game.phase != GamePhase::Finished`を確認（既にゲーム終了なら`commit_game`へ）
2. ディーラー交代: `game.dealer_position = 1 - game.dealer_position`
3. `game.player1_committed = 0`, `game.player2_committed = 0`をリセット
4. `game.last_raise_amount = 0`をリセット
5. `game.betting_closed = false`（次のハンドのベット受付を再開）
6. 次のハンドの`shuffle_and_deal`呼び出しを待機（`phase = Waiting`のまま）

**注**: ブラインドレベルの更新は`shuffle_and_deal`内でのみ行う（二重更新を避けるため）。`shuffle_and_deal`がhand_numberをインクリメントした後にブラインドスケジュールを適用する。

#### 4.2.6 `commit_game(game_id: u64)`
...
**処理**:
1. `game.phase == GamePhase::Finished`を確認（`settle_hand`が完了している必要がある）
   - **ハンド評価は`settle_hand`内で実行済みのため、ここでは行わない**
2. `game.winner`が設定済みであることを確認（`settle_hand`で設定される）
3. MagicBlock `commit_and_undelegate` CPIを呼び出し:
...
- hand_numberを混合することで各ハンドで必ず異なるデッキを生成（同一game_idの再利用を防止）

### 7.2 Fisher-Yatesシャッフル

```rust
pub fn shuffle_deck(seed: &[u8; 32]) -> Result<[u8; 52]> {
    ...
    for i in (1..52).rev() {
        ...
        let bytes: [u8; 8] = rng_state[0..8]
            .try_into()
            .map_err(|_| PokerError::InvalidAction)?;
...
ハンドタイプごとに異なるエンコーディングを使用し、同ランクハンドを正しく比較する:

| ハンドタイプ | エンコーディング | 備考 |
|---|---|---|
| HighCard / Flush | `encode_kickers(ranks)` | 5枚を4bitずつ降順パック |
| OnePair(u32) | `(pair << 12) \| (k1 << 8) \| (k2 << 4) \| k3` | ペアランク + キッカー3枚降順 |
| TwoPair(u32) | `(high_pair << 8) \| (low_pair << 4) \| kicker` | 高ペア優先 → 低ペア → キッカー |
| ThreeOfAKind(u32) | `(trips << 8) \| (k1 << 4) \| k2` | トリップスランク + キッカー2枚 |
| FullHouse(u16) | `(trips << 4) \| pair` | トリップスランク + ペアランク |
| FourOfAKind(u16) | `(quad << 4) \| kicker` | クワッドランク + キッカー |
| Straight / StraightFlush | `u8`（最高ランク） | — |

```rust
// HighCard・Flush専用: 5枚のカードランクを降順にソートし、各4ビットにパック
...
// C(7,5)=21通りの組み合わせをヒープ不使用で生成
fn combinations_5_of_7(cards: &[u8; 7]) -> [[u8; 5]; 21] {
    ...
    let mut result = [[0u8; 5]; 21];
    for (i, c) in COMBOS.iter().enumerate() {
        result[i] = [cards[c[0]], cards[c[1]], cards[c[2]], cards[c[3]], cards[c[4]]];
    }
    result
}

// 5枚のカードを評価してHandRankを返す
// card_value = rank(0-12) + suit(0-3)*13
fn evaluate_five_cards(cards: &[u8; 5]) -> HandRank {
    let ranks: [u8; 5] = cards.map(|c| c % 13);
    ...
    // ランク別頻度カウント（高ランクから降順に処理）
    let mut freq = [0u8; 13];
    for &r in &ranks {
        freq[r as usize] += 1;
    }

    let mut quads_rank: Option<u8> = None;
    let mut trips_rank: Option<u8> = None;
    let mut pairs = [0u8; 2];
    ...
    for r in (0..13u8).rev() {
        match freq[r as usize] {
            4 => quads_rank = Some(r),
            3 => trips_rank = Some(r),
            2 if pair_count < 2 => { pairs[pair_count] = r; pair_count += 1; }
            1 if single_count < 5 => { singles[single_count] = r; single_count += 1; }
            _ => {}
        }
    }

    if let Some(q) = quads_rank {
        // singles[0] は唯一のキッカー（降順ループ済み）
        return HandRank::FourOfAKind(((q as u16) << 4) | singles[0] as u16);
    }

    if let Some(t) = trips_rank {
        if pair_count > 0 {
            return HandRank::FullHouse(((t as u16) << 4) | pairs[0] as u16);
        }
        let v = ((t as u32) << 8) | ((singles[0] as u32) << 4) | singles[1] as u32;
        return HandRank::ThreeOfAKind(v);
    }

    if pair_count == 2 {
        // pairs[0] > pairs[1]（降順ループ保証）
        let v = ((pairs[0] as u32) << 8) | ((pairs[1] as u32) << 4) | singles[0] as u32;
        return HandRank::TwoPair(v);
    }

    if pair_count == 1 {
        let v = ((pairs[0] as u32) << 12)
            | ((singles[0] as u32) << 8)
            | ((singles[1] as u32) << 4)
            | singles[2] as u32;
        return HandRank::OnePair(v);
    }

    if is_flush {
        return HandRank::Flush(encode_kickers(&ranks));
    }
    ...
    HandRank::HighCard(encode_kickers(&ranks))
}
```
...
fn check_straight(ranks: &[u8; 5]) -> Option<u8> {
    ...
    // 重複があればストレートではない（[u8; 5]はdedup後もlen()が変わらないため明示チェック）
    ...
    // ホイール（A-2-3-4-5）判定: sorted = [0, 1, 2, 3, 12] (2,3,4,5,A)
    if sorted[0] == 0 && sorted[1] == 1 && sorted[2] == 2
        && sorted[3] == 3 && sorted[4] == 12
    {
        return Some(3); // 5（rank=3）がハイカード
    }
...
### 12.2 ゲーム進行フロー（TEE）- マルチハンド

```
=== ハンド N (N=1,2,...) ===

Step A:   shuffle_and_deal(game_id, seed)   [クランク: hand_number++, ブラインドレベル更新, ブラインド投入]
--- PreFlop (SBが先手) ---
Step B:   player_action(game_id, ...)       [SBプレイヤー]
Step C:   player_action(game_id, ...)       [BBプレイヤー]
          ... (ベッティングラウンド終了まで繰り返し)
          ↓ Fold発生 → Step M へ
          ↓ All-in + Call → Showdownへスキップ（Step J', K'）

Step D:   reveal_community_cards(game_id, Flop)  [current_turn = BBに切替]
--- Flop (BBが先手) ---
Step E-F: player_action(game_id, ...)       [BBから交互にアクション]
          ↓ Fold発生 → Step M へ

Step G:   reveal_community_cards(game_id, Turn)  [current_turn = BBに切替]
--- Turn (BBが先手) ---
Step H-I: player_action(game_id, ...)
          ↓ Fold発生 → Step M へ

Step J:   reveal_community_cards(game_id, River) [current_turn = BBに切替]
--- River (BBが先手) ---
Step K-L: player_action(game_id, ...)
          ↓ Fold発生 → Step M へ
          ↓ ベッティング完了（Foldなし）→ game.phase = Showdown → Step M へ

[All-in + Call発生時のショートカット]
Step J':  reveal_community_cards(game_id, Turn)   [残りカード公開]
Step K':  reveal_community_cards(game_id, River)
          → game.phase = Showdown → Step M へ

Step M:   settle_hand(game_id)    [TEE] ← ★ハンド評価・チップ精算・effective stack調整をここで実行
          ↓ いずれかのchip_stack == 0 → Step Q (commit_game)
Step N:   start_new_hand(game_id) [TEE: ディーラー交代]
          ↓ 次のハンドへ (Step A に戻る)

=== ゲーム終了 ===

Step Q:   commit_game(game_id)    [TEE: settle_hand完了後、commit_and_undelegate]
```

**チェックポイント（50ハンドごと）**:
- `hand_number % 50 == 0`の場合、`commit_game` → `resolve_game`の代わりにL1チェックポイントを実行
- チェックポイント後に再デリゲートして続行（詳細はマルチハンド設計参照）

### 12.3 決済フロー（L1）

```
Step R:   resolve_game(game_id)            [勝者へSOL転送（winner-take-all - 2%手数料）]
Step S:   claim_betting_reward(game_id)    [各観戦者が個別にクレーム]
```
...
...
↓ TEEでAll-in発生 → game.betting_closed = true
  ゲームサーバーが検知 → close_betting_pool(game_id)  [L1: BettingPool締め切り]
  以降の place_spectator_bet は BettingClosed エラーで拒否される
```

**締め切りフロー詳細**:
1. TEE内で`player_action(AllIn)`が実行される
2. Gameの`betting_closed = true`がTEE内で設定される
3. ゲームサーバーがTEEのイベントストリームでこれを検知
4. ゲームサーバーがL1で`close_betting_pool(game_id)`を呼び出す
5. `betting_pool.is_closed = true`に設定（以降のベットを拒否）
